# Performance extraction script

Use this from an authenticated Koyfin browser session on the `Performance` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/performance/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Performance`, paste `extract.js`, and copy the JSON/CSV result.
