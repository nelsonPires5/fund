# Profitability extraction script

Use this from an authenticated Koyfin browser session on the `Profitability` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/profitability/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Profitability`, paste `extract.js`, and copy the JSON/CSV result.
