# Solvency extraction script

Use this from an authenticated Koyfin browser session on the `Solvency` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/solvency/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Solvency`, paste `extract.js`, and copy the JSON/CSV result.
