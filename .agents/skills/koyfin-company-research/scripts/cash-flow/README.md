...
Use this from an authenticated Koyfin browser session on the `Cash Flow` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/cash-flow/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Cash Flow`, paste `extract.js`, and copy the JSON/CSV result.
