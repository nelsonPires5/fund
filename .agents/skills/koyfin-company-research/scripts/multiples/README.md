# Multiples extraction script

Use this from an authenticated Koyfin browser session on the `Multiples` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/multiples/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Multiples`, paste `extract.js`, and copy the JSON/CSV result.
