# Historical extraction script

Use this from an authenticated Koyfin browser session on the `Historical` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/historical/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Historical`, paste `extract.js`, and copy the JSON/CSV result.
