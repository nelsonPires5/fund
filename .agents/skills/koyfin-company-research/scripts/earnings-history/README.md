# Earnings History extraction script

Use this from an authenticated Koyfin browser session on the `Earnings History` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/earnings-history/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Earnings History`, paste `extract.js`, and copy the JSON/CSV result.
