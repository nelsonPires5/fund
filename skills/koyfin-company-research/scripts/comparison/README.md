# Comparison extraction script

Use this from an authenticated Koyfin browser session on the `Comparison` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/comparison/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Comparison`, paste `extract.js`, and copy the JSON/CSV result.
