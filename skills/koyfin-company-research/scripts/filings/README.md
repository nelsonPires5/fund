# Filings extraction script

Use this from an authenticated Koyfin browser session on the `Filings` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/filings/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Filings`, paste `extract.js`, and copy the JSON/CSV result.
