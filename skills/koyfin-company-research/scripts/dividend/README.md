# Dividend extraction script

Use this from an authenticated Koyfin browser session on the `Dividend` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/dividend/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Dividend`, paste `extract.js`, and copy the JSON/CSV result.
