# News extraction script

Use this from an authenticated Koyfin browser session on the `News` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/news/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `News`, paste `extract.js`, and copy the JSON/CSV result.
