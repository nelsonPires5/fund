# Transcripts extraction script

Use this from an authenticated Koyfin browser session on the `Transcripts` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/transcripts/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Transcripts`, paste `extract.js`, and copy the JSON/CSV result.
