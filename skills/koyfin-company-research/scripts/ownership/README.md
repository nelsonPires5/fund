# Ownership extraction script

Use this from an authenticated Koyfin browser session on the `Ownership` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/ownership/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Ownership`, paste `extract.js`, and copy the JSON/CSV result.
