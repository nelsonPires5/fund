# Actuals and Consensus extraction script

Use this from an authenticated Koyfin browser session on the `Actuals and Consensus` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/actuals-and-consensus/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Actuals and Consensus`, paste `extract.js`, and copy the JSON/CSV result.
...
Example files from MSFT exploration: sample-output.csv, sample-output.json.
