# Press Releases extraction script

Use this from an authenticated Koyfin browser session on the `Press Releases` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/press-releases/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Press Releases`, paste `extract.js`, and copy the JSON/CSV result.
