...
Use this from an authenticated Koyfin browser session on the `Balance Sheet` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/balance-sheet/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Balance Sheet`, paste `extract.js`, and copy the JSON/CSV result.
