# Income Statement extraction script

Use this from an authenticated Koyfin browser session on the `Income Statement` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/income-statement/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Income Statement`, paste `extract.js`, and copy the JSON/CSV result.
