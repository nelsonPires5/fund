# ROIC extraction script

Use this from an authenticated Koyfin browser session on the `ROIC` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/roic/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `ROIC`, paste `extract.js`, and copy the JSON/CSV result.
