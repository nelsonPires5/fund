# Price Target extraction script

Use this from an authenticated Koyfin browser session on the `Price Target` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/price-target/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Price Target`, paste `extract.js`, and copy the JSON/CSV result.
