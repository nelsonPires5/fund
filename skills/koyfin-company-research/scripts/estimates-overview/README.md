# Estimates Overview extraction script

Use this from an authenticated Koyfin browser session on the `Estimates Overview` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/estimates-overview/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Estimates Overview`, paste `extract.js`, and copy the JSON/CSV result.
