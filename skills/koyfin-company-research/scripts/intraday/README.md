# Intraday extraction script

Use this from an authenticated Koyfin browser session on the `Intraday` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/intraday/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Intraday`, paste `extract.js`, and copy the JSON/CSV result.
