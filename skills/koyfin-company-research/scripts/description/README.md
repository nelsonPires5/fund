...
Use this from an authenticated Koyfin browser session on the `Description` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/description/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Description`, paste `extract.js`, and copy the JSON/CSV result.
