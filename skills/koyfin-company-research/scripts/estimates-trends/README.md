# Estimates Trends extraction script

Use this from an authenticated Koyfin browser session on the `Estimates Trends` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/estimates-trends/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Estimates Trends`, paste `extract.js`, and copy the JSON/CSV result.
