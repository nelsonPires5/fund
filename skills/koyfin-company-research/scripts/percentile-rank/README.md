# Percentile Rank extraction script

Use this from an authenticated Koyfin browser session on the `Percentile Rank` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/percentile-rank/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Percentile Rank`, paste `extract.js`, and copy the JSON/CSV result.
