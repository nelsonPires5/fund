# Highlights extraction script

Use this from an authenticated Koyfin browser session on the `Highlights` tab for the target ticker.
...
code = open('skills/koyfin-company-research/scripts/highlights/extract.js').read()
print(js(code))
PY
```
...
Open Koyfin → Security Analysis → `Highlights`, paste `extract.js`, and copy the JSON/CSV result.
